...
class EfficiencyModule(Module):
	...
	for genpart in Genparts:
		...
		if(abs(PDG)==5):
		    print abs(PDG)," -> ",
		    #b_quark_id = genpart.genPartIdx
		    if(abs(motherPDG)!=6):
			print abs(motherPDG)," -> ",
			printonce = False
		    	while (abs(GmotherPDG)!=6 and motherPDG == GmotherPDG and GmotherPDG !=-1 and motherPDG !=-1 and Gmotheridx!=-1):
			     if(printonce==False): print abs(GmotherPDG)," -> ",
			     PDG = motherPDG
