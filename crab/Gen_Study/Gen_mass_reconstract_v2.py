...
class EfficiencyModule(Module):
	...
	for genpart in Genparts:
		...
		if(abs(PDG)==5):
			...
			#print "mother is top "
		    elif(abs(motherPDG)==abs(PDG) and abs(GmotherPDG)==6) and top_from_bquark[0]!=abs(Gmotheridx):
			top_from_bquark.append(Gmotheridx)
			#print "Gmother is top"		
		    #print top_from_bquark		
	
		if(abs(PDG)==11 or abs(PDG)==13):
		    if( abs(motherPDG)==24 or abs(motherPDG)==11 or abs(motherPDG)==13):	
		    	while ((motherPDG == GmotherPDG and GmotherPDG !=-1 and motherPDG !=-1 and Gmotheridx!=-1 and abs(GmotherPDG)!=6) or ( abs(motherPDG)==24 and abs(GmotherPDG)!=6 )):
			     PDG = motherPDG
			     motherPDG,Gmotheridx,GmotherPDG = findMother(Gmotheridx,Genparts)
			     #print("-----> ",motherPDG,Gmotheridx,GmotherPDG)	
