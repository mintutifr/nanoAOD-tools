...
class UserCacheHelper:
    def __init__(self, proxy=None, user=None):
        if proxy is None:
	    #os.system("export X509_USER_PROXY=/tmp/x509up_u106474")
            proxy = os.getenv('X509_USER_PROXY') #export X509_USER_PROXY=/tmp/x509up_u106474
        if not proxy or not os.path.isfile(proxy):
            raise Crab3ToolsException('X509_USER_PROXY is %r, get grid proxy first export X509_USER_PROXY=/tmp/x509up_u106474' % proxy)
        self.proxy = proxy
    ...
    def listusers(self):
        return self._curl('https://cmsweb-k8s-prod.cern.ch/crabcache/info?subresource=listusers')

    def userinfo(self):
        return self._only(self._curl('https://cmsweb-k8s-prod.cern.ch/crabcache/info?subresource=userinfo&username=' + self.user))
    ...
    def fileinfo(self, hashkey):
        return self._only(self._curl('https://cmsweb-k8s-prod.cern.ch/crabcache/info?subresource=fileinfo&hashkey=' + hashkey))
    ...
    def fileremove(self, hashkey):
        x = self._only(self._curl('https://cmsweb-k8s-prod.cern.ch/crabcache/info?subresource=fileremove&hashkey=' + hashkey))
        if x:
...
if __name__ == '__main__':
    ...
    for x in h.filelist():
        if '.log' in x:
            continue
        print 'remove', x
	#if(x!="dry-run-sandbox.tar.gz"): h.fileremove(x)
	h.fileremove(x)
