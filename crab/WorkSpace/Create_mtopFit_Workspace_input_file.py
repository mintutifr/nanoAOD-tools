...
def get_Weight_sys_top_JES(lep,year,Variable,channels_weight_sys,MCcut,DNNcut,hist_sys_name,File_with_mtwMassFit_weight_Iso,Fpaths_DNN_apply,top_sig_cons,top_bkg_cons,Fpaths_sys_samples="None"):
    QCDcut=""
    Datacut=""
...
def get_Weight_sys_EWK_JES(lep,year,Variable,channels_weight_sys,MCcut,DNNcut,hist_sys_name,File_with_mtwMassFit_weight_Iso,Fpaths_DNN_apply,EWK_cons,Fpaths_sys_samples="None"):
    ...
    propagate_rate_uncertainity(EWK_weight_sys, EWK_cons)
    EWK_weight_sys.Print()
...
def Create_Workspace_input_file(lep="mu",year="UL2017",Variable="lntopMass"):
    ...
    if(allsys or JES_JERSys):
        ...
        sys_JES = ['jesAbsoluteStat','jesAbsoluteMPFBias']
        sys_variation = ["Up","Down"]
        channel_JES = ["Tchannel"]
        Fpaths_sys_samples_top,Fpaths_sys_samples_EWK = {},{}
        for channel in channels_top_only:
            Fpaths_sys_samples_top[channel] = "/feynman/home/dphp/mk277705/work/RUN2_UL/Minitree_corr_bweight/JER_JES_Trees/JERJEStree_"+year+"_"+channel+"_2J1T1_"+lep+".root"
        for channel in channels_EWK_only:
            Fpaths_sys_samples_EWK[channel] = "/feynman/home/dphp/mk277705/work/RUN2_UL/Minitree_corr_bweight/JER_JES_Trees/JERJEStree_"+year+"_"+channel+"_2J1T1_"+lep+".root"
        ...
        for sys in sys_JES:
            for variation_no,variation in enumerate(sys_variation):
                ...
                topSig_hists_JES_syst,topBkg_hists_JES_syst = get_Weight_sys_top_JES(
                    lep=lep,
                    year=year,
                    Variable=Variable,
                    ...
                    Fpaths_DNN_apply=Fpaths_DNN_apply,top_sig_cons=top_sig_cons,
                    top_bkg_cons=top_bkg_cons,
                    Fpaths_sys_samples = Fpaths_sys_samples_top
                )
                topSig_hists_JES_syst.SetName("top_sig_1725"+tag+gt_or_lt_tag+sys+variation)
                topBkg_hists_JES_syst.SetName("top_bkg_1725"+tag+gt_or_lt_tag+sys+variation)
                hist_to_return.append(topSig_hists_JES_syst.Clone())
                hist_to_return.append(topBkg_hists_JES_syst.Clone())
                del topSig_hists_JES_syst,topBkg_hists_JES_syst
                EWK_hists_syst_JES = get_Weight_sys_EWK_JES(
                    lep=lep,
                    year=year,
                    Variable=Variable,
                ...
                sys_EWK_Integral = EWK_hists_syst_JES.Integral()
                EWK_hists_syst_JES = Control_EWK_hist.Clone()
                EWK_hists_syst_JES.Scale(sys_EWK_Integral/control_region_EWK_Integral)
                EWK_hists_syst_JES.SetName("EWK_bkg"+tag+gt_or_lt_tag+sys)
