    ...
    for file in `xrdfs se01.indiacms.res.in ls /cms$t2_dir | grep \\root | awk '{print $1}'`; do
        ...
        if [ $flag -eq 0 ]; then
			if [ $blockCount -ne 0 ]; then
				...
				echo -e "hadd ${tmpMerge_name[$((blockID - 1 ))]} \t ${array_name[*]} \t root://cms-xrd-global.cern.ch/${file:4}"
#				hadd ${tmpMerge_name[$((blockID - 1 ))]} ${array_name[*]} root://cms-xrd-global.cern.ch/$t2_dir$file
				python haddnano.py ${tmpMerge_name[$((blockID - 1 ))]} ${array_name[*]} root://cms-xrd-global.cern.ch/${file:4}	
			fi
		else
...
#					hadd ${tmpMerge_name[$((blockID - 1 ))]} ${array_name[*]} root://cms-xrd-global.cern.ch/$t2_dir$file
					...
					echo -e "hadd ${tmpMerge_name[$((blockID - 1 ))]} \t ${array_name[*]:0:$((blockCount - 1 ))} \t root://cms-xrd-global.cern.ch/${file:4}"
#					hadd ${tmpMerge_name[$((blockID - 1 ))]} ${array_name[*]} root://cms-xrd-global.cern.ch/$t2_dir$file
					python haddnano.py ${tmpMerge_name[$((blockID - 1 ))]} ${array_name[*]:0:$((blockCount - 1 ))} root://cms-xrd-global.cern.ch/${file:4}
				fi		
			fi	
		fi
